...
out_folder = r'C:\Users\andre\ScatterPlots\Single_Line_Convergence'
os.makedirs(out_folder, exist_ok=True)

# Restricting exclusively to the 110m Single Line data
csv_files = {
    '110m_Single_Line': 'SRER_NRI_Plots_110m_Single_Line_Convergence.csv'
}
...
if __name__ == '__main__':
    for scale_name, csv_filename in csv_files.items():
        ...
        # Extract the average number of pins using the updated 'Line_' prefix
        pin_counts = [df[f'Line_TotalPins_{inv}cm'].mean() for inv in LINE_INTERVALS_CM]
        
        # Updated to point to the Single Line columns generated in the previous step
        metrics = {
            'Bare Ground Percentage': ('Line_BGR_{}cm_Pct', 'Exact_BGR_Pct'),
            'Herb Cover Percentage': ('Line_Herb_{}cm_Pct', 'Exact_Herb_Pct'),
            'Woody Cover Percentage': ('Line_Woody_{}cm_Pct', 'Exact_Woody_Pct'),
            'Herb-to-Woody Ratio': ('Line_HW_Ratio_{}cm', 'Exact_HW_Ratio'),
            'Mean Fetch': ('Line_Fetch_{}cm', 'Exact_Fetch_m')
        }
        ...
        fig.suptitle(f'Diagnostic Convergence ({scale_name})', fontsize=16, fontweight='bold', y=0.99)
