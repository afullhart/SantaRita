...
feature_classes_to_process = {
    os.path.join(out_gdb, 'SRER_NRI_Plots_110m'): 'SRER_NRI_Plots_110m_Spoke_Convergence.csv',
    os.path.join(out_gdb, 'SRER_Grid_30m'): 'SRER_Grid_30m_Spoke_Convergence.csv',
    os.path.join(out_gdb, 'SRER_Grid_10m'): 'SRER_Grid_10m_Spoke_Convergence.csv'
}
...
# The intervals we want to test in centimeters (5cm = continuous pixels)
...
def process_polygon(task):
    ...
    try:
        ...
        # --- SYNTHETIC NRI SPOKES (DYNAMIC INTERVALS) ---
        ...
        nri_data = {inv: {'bgr': 0, 'herb': 0, 'woody': 0, 'pins': 0, 'fetch_vals': []} for inv in NRI_INTERVALS_CM}
        ...
        for az in [0, 120, 240]:
            ...
            for interval in NRI_INTERVALS_CM:
                step = max(1, int((interval / 100.0) / cell_size))
                vals = transect_values[::step]
                
                nri_data[interval]['bgr'] += np.sum(vals == target_value)
                nri_data[interval]['herb'] += np.sum(vals == herb_value)
                nri_data[interval]['woody'] += np.sum(vals == woody_value)
                nri_data[interval]['pins'] += len(vals)
                nri_data[interval]['fetch_vals'].extend(fetch_transect_values[::step])
        
        # Calculate percentages and append to results
        for interval in NRI_INTERVALS_CM:
            pins = nri_data[interval]['pins']
            bgr_pct = (float(nri_data[interval]['bgr']) / pins) * 100 if pins > 0 else 0.0
            herb_pct = (float(nri_data[interval]['herb']) / pins) * 100 if pins > 0 else 0.0
            woody_pct = (float(nri_data[interval]['woody']) / pins) * 100 if pins > 0 else 0.0
            hw_ratio = (float(nri_data[interval]['herb']) / float(nri_data[interval]['woody'])) if nri_data[interval]['woody'] > 0 else None
            
            f_vals = np.array(nri_data[interval]['fetch_vals'])
...
if __name__ == '__main__':
    ...
    for interval in NRI_INTERVALS_CM:
        new_fields.extend([
            f'NRI_BGR_{interval}cm_Pct', f'NRI_Herb_{interval}cm_Pct', f'NRI_Woody_{interval}cm_Pct',
            f'NRI_HW_Ratio_{interval}cm', f'NRI_Fetch_{interval}cm', f'NRI_TotalPins_{interval}cm'
        ])
    ...
    for current_fc, csv_name in feature_classes_to_process.items():
        ...
        # We will build a temporary feature class in memory to hold this specific experiment
        # so we don't pollute your main Geodatabase feature classes.
